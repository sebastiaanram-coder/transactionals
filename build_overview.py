...
FLOWS = [
 dict(slug="welcome", name="Welcome Flow", stage="Acquire", flow_id="RXBWV9",
   ...
   reentry="Not set", incentive="10% off first order · code HELLO10 · 14-day expiry", cadence="Day 0, then one email per day for 3 days",
   ...
   flow_flags=["Two variants per email, chosen by a conditional split. Every email is preceded by a check on Placed Order since starting this flow. Zero orders gets the discount version; one or more gets a version with the offer stripped, so a customer who buys mid-series finishes onboarding instead of being dropped. This replaces the current message-level filter, which suppresses the send entirely and would make the second branch unreachable.",
   ...
   emails=[
    ...
    dict(step=2, when="Day 1", subject="The people behind your print", preview="Printed closer to you, a B Corp certification, and over 10,000 products.",
      goal="Build trust through the humans behind the product: local team, B Corp status, Trustpilot score. Softly repeat the 15% code.",
    ...
    dict(step=3, when="Day 2", subject="Rated Excellent on Trustpilot", preview="4.5 from more than 34,000 reviews. Here is what a few of them say.",
      goal="Convert with proof: review score, all-inclusive pricing and the guarantee. Removes the risk argument for a first order.",
      who="Same audience, two days in, still no order.", tpl="RpQvJH", flags=[], badge=None,
      final="welcome-03-proposed.html"),
    dict(step=4, when="Day 3", subject="Send it over, we'll handle it", preview="Artwork or just an idea. Send either and John's team takes it from there.",
      goal="Show the breadth of the catalogue plus the custom-work route; last push for the intro offer.",
      who="Same audience, final email of the series.", tpl="XVPf5F", flags=[], badge=None,
...
WELCOME_DETAIL = {
 "Vb23CK": dict(
   ...
   variant="Barely affected in practice: this fires within minutes of sign-up, so almost nobody will have ordered yet. If they have, the promo bar, the code chip, the \u201cyour 10% comes off at checkout\u201d line and the discounted figure under every tile price all come out, the price returns to brand green, and the CTA changes from \u201cStart your first order\u201d to a plain browse. Both files exist in the repo \u2014 proposals/welcome-01-proposed.html and proposals/welcome-01-proposed-nodiscount.html \u2014 and the second is generated from the first by scripts/make-nodiscount.py, so the two can never drift apart by hand.",
   elements=[
     ("Promo bar with the code.", "Delivers what they signed up for inside the first 40 pixels, and puts a clock on it."),
     ...
     ("Product grid: image, title, quantity, price, price with the code.", "Removes the navigation step at peak intent and puts real prices in front of someone who has never seen one. The second figure shows what the code actually achieves on that exact quantity, in green under the catalogue price \u2014 not a struck-through was/now, because the catalogue price is not being reduced. Each tile click is also a category signal, which is the only thing that can make Browse Abandonment relevant later."),
 ...
 "TtjyZ4": dict(
   ...
   elements=[
     ("Softened promo bar.", "The offer becomes a reminder rather than the headline, which frees the email to do its actual job."),
 ...
 "RpQvJH": dict(
   ...
   why="By day 2 they have heard our offer and our story. The one thing they have not heard is anybody other than us. Third-party proof is the strongest lever left, and it has to land before the code expires.",
 ...
 "XVPf5F": dict(
   ...
   why="Final email, code expiring. Everyone reading it has seen the offer, the brand and the proof and still not bought, which means self-serve did not work for them. A third catalogue push would add nothing. The lowest-friction action left is a conversation, and a reply is a lead we can work where a non-open is nothing at all.",
   variant="Promo bar goes neutral, and the first-order framing softens: the adviser introduction still works for a second order, but \u201clast chance\u201d and \u201cyour first order\u201d do not.",
   elements=[
