    ...
    dict(step=3, when="Day 32 \u00b7 Category nudge", subject="One email, five categories",
      ...
      variants=[
        dict(key="cp", label="Commercial Print", subject="What are you promoting next?",
             ...
             final="category-commercial-print-proposed.html"),
        dict(key="st", label="Stationery", subject="Running low on office stationery?",
             preview="The print that runs out quietly, and what to top up.",
             final="category-stationery-proposed.html"),
...
# which version it is.
VERSION = "v1.1"
VERSION_DATE = "31 Aug 2026"

ISSUES = [
 "Every email in the programme is translated into six languages across nine locales: English, Dutch, French, German, Spanish and Italian, with nl-BE reading Dutch and fr-BE reading French unless a string is given its own override. 495 keys, one universal HTML block per email carrying all of them behind a switch on event.Locale, and 145 rendered previews so a colleague proofreads an email rather than a spreadsheet. Use the Proofread in bar above any preview. What is NOT done is the native-speaker pass: these are my translations, and nobody whose first language it is has read them yet.",
 ...
 "All six category nudges are built, all six have a landing page and all six have a header photograph. Five are fully photographed to the tile, twenty-two in-setting shots and no packshots on white, and Clothing & Textiles and Corporate Gifts also carry a band of three in-setting brand shots from the carousel on /our-brands. The sixth, Stationery, still needs four tile photographs: Notepads, Envelopes, Folders and Letterheads. Until they exist those four tiles fall back to Contentful packshots, which look out of place beside the header.",
 "Stationery is the only nudge that is a split of another category rather than a category of its own, which makes two things unusual about it. Its flow condition tests level two of Categories rather than level one, so it has to be evaluated before Commercial Print, which now excludes it. And Letterheads, worth 179k and the fourth tile, has no URL of its own in en-IE or it-IT: those two would have fallen back to a UK page, so the correct URLs are patched in from CURL_OVERRIDE in scripts/fetch_subcategories.py. Both were checked and return 200. That patch should be deleted the moment the two Contentful fields are filled, and every fetch prints which overrides it used so it cannot be forgotten.",
    ...
    if e.get("final"):
        ...
        def lang_bar(key):
            ...
            return (f'<div class="lgbar" data-key="{key}">'
                    f'<span class="lgbar-t">Proofread in</span>{btns}'
                    f'<span class="lgbar-n">Product names stay English in the '
                    f'previews. They are placeholders: the real email takes them '
                    f'from the localised catalogue and from the order event, so '
                    f'they arrive in the reader\u2019s language at send time.</span>'
                    f'</div>')
...
.lgbar-n{flex:1 1 100%;font-size:11px;line-height:16px;color:var(--ink3);margin:2px 0 0}
