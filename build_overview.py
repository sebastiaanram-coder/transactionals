...
EMAIL_DETAIL = {
 ...
 "POST04": dict(
   ...
   elements=[
     ...
     ("John signs it, with the block from his Abandoned Order note.", "Same avatar, same name and role, laid out as a signature rather than as a card heading, under a hairline that makes it read as a sign-off. He is recognisably the same person across the two flows. His address is in the signature, so the footer no longer repeats it."),
