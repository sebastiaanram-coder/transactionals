...
ISSUES = [
 "THE EVENT PAYLOADS BLOCK MORE OF THIS PROGRAMME THAN ANYTHING ELSE, and none of it is a design question. Measured on the 100 most recent events of each type: Locale is completely absent from Started Checkout and Viewed Product, 0 of 100 each, which is why the six abandoned-order and three browse emails carry hardcoded /en-ie/ links and can only run in Ireland. Categories is absent from every v4 Placed Order event and missing on 30% of presta, so the five category nudges have nothing to route on. Written up for the payload owners in proposals/event-payload-handover.md, with the field names, formats and acceptance criteria.",
 ...
 "All five category nudges are built, all five have a landing page, and every one of their twenty-two images is an in-setting shot rather than a packshot on white. Clothing & Textiles and Corporate Gifts also carry a band of three in-setting brand shots taken from the carousel on /our-brands. Nothing is now waiting on photography: the last gap, Cards & Invitations in Commercial Print, is filled and that tile is back to being the fourth by gross profit at 261k rather than a roller banner at 175k standing in for it.",
