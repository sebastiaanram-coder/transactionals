...
WELCOME_DETAIL = {
 "Vb23CK": dict(
   ...
   variant="Barely affected in practice: this fires within minutes of sign-up, so almost nobody will have ordered yet. If they have, the promo bar, the code chip, the \u201cyour 10% comes off at checkout\u201d line and the second figure beside every tile price all come out, the strike-through goes with them and the price returns to brand green, and the CTA changes from \u201cStart your first order\u201d to a plain browse. Both files exist in the repo \u2014 proposals/welcome-01-proposed.html and proposals/welcome-01-proposed-nodiscount.html \u2014 and the second is generated from the first by scripts/make-nodiscount.py, so the two can never drift apart by hand.",
   elements=[
