...
FLOWS = [
 ...
 dict(slug="abandoned-cart", name="Abandoned Order", stage="Convert", flow_id="VCVzm6",
   ...
   reentry="14 days",
   incentive="Split by cart value. High value: no code until email 3, then 10% for 72 hours. Low value: 10% in email 2, rising to 15% for the final 24 hours.",
   cadence="Proposed: 1 hour, 24 hours, 72 hours. Three emails on each branch, down from five in four days across two flows.",
   mail_line="0 of 3 rebuilt \u00b7 split by cart value \u00b7 5 RFB emails kept below",
   flow_note="This is the merged flow, and it splits by cart value at entry: at 150, in either currency, about a quarter of carts go to a high-value branch that leads with a print expert, and the rest to a low-value branch that leads with the incentive. RFB ran four separate abandonment journeys — Site, Browse, Cart and Checkout — whose copy was largely interchangeable; Cart and Checkout were near-identical apart from the trigger. The rebuild runs two: Browse Abandonment for a product view, and this one for anything that reached a basket. Site Abandonment is dropped entirely, because a visit with no product view carries too little intent to say anything useful about. The five RFB emails below are the current state and have not been rebuilt yet.",
   emails=[
