    ...
    dict(step=3, when="Day 32 \u00b7 Category nudge", subject="One email, five categories",
      ...
      variants=[
        dict(key="cp", label="Commercial Print", subject="What are you promoting next?",
             ...
             final="category-commercial-print-proposed.html"),
        dict(key="st", label="Stationery", subject="Running low on any of it?",
...
ISSUES = [
 ...
 "All six category nudges are built and all six have a landing page. Five of them are fully photographed, twenty-two in-setting shots and no packshots on white, and Clothing & Textiles and Corporate Gifts also carry a band of three in-setting brand shots from the carousel on /our-brands. The sixth, Stationery, needs four photographs: Notepads, Envelopes, Folders and Bookmarks. Until they exist its tiles fall back to Contentful packshots and it has no header photograph at all, so it runs the plain wordmark header the other five started with.",
 "Stationery is the newest nudge and the only one that is a split of another category rather than a category of its own. Two things are outstanding beyond photography. Letterheads belongs in the grid at 179k of gross profit but has no URL of its own in en-IE or it-IT, so it would send Irish and Italian readers to a UK page: Bookmarks stands in at 96k until those two Contentful fields are filled, and the build refuses to ship the swap until they are. And it is the only email whose flow condition tests level two of Categories rather than level one, so it has to be evaluated before Commercial Print, which now excludes it.",
