...
# flat exact-match switch on person.locale, the mechanism every template here
...
# halve the size:
#
#     {% if person.locale == 'nl-NL' or person.locale == 'nl-BE' %}
#
...
def review_block(P, cat, live, locale=None, tr=None):
    ...
    chain on person.locale, the mechanism every other switch here uses and the one
    ...
    for i, loc in enumerate(i18n.LOCALES):
        ...
        out += "{%% %s %s == '%s' %%}%s" % (
            "if" if i == 0 else "elif", i18n.LOCALE_EXPR, loc, body)
...
<!-- HP - Post-Purchase - category nudge - %(label)s
     Preview shows the en-IE names and URLs. Live build switches on person.locale.
...
KLAVIYO_DOC = """<!--
  ...
  NAME AND URL SWITCH ON person.locale, mapped to the Contentful locale:
...
for cat in CATEGORIES:
    ...
    # its own double quotes - href="{% if person.locale == "en-IE" %}https://..." -
