...
def tp_switch(live):
    ...
    for email_loc, cf_loc in sc.LOCALE_MAP.items():
        ...
        out += "{%% %s %s == '%s' %%}%s" % (
            kw, i18n.LOCALE_EXPR, email_loc, rv.write_url(cf_loc))
...
def build(e, live, locale=None):
    ...
    score = ""
    if e["score"]:
        # THE NUMBERS ARE FILLED AFTER TRANSLATION, which means filling EVERY
        # branch of the switch, not the first one.
        #
        # This used to say .replace("%s", ..., 1), which does exactly what the
        # comment above it warned against: count=1 applies to the whole assembled
        # nine-way switch, so only the en-IE branch got its numbers and the other
        # eight plus the {% else %} shipped the literal characters "%s" to the
        # reader. 18 of 20 placeholders survived into the built email.
        #
        # Named tokens instead of %s, replaced with no count, so the number of
        # branches cannot matter. Same reason {n}, {amt} and {h} are used
        # elsewhere: a translated string is a switch, not a format string.
        nums = (rv.score(), rv.review_total() // 1000 * 1000)
        score = ('<p class="%s-scoretx">%s</p>'
                 % (P, tr("review.score", SHARED["score"], fills={
...
<!-- HP - Post-Purchase - %(step)02d - %(label)s - day %(day)d
     Preview shows the Irish review link. Live build switches on person.locale.
...
for e in EMAILS:
    ...
    _tp = "{%% if %s == '%s' %%}%s" % (
        i18n.LOCALE_EXPR, _first, rv.write_url(sc.LOCALE_MAP[_first]))
